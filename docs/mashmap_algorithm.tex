...
\title{WFMash: A Pangenome-Scale Aligner Based on MashMap 3.5}
...
WFMash is a pangenome-scale aligner designed to make whole genome alignment easy and efficient, even for gigabase-scale genomes with high sequence divergence. At its core, WFMash uses MashMap 3.5 for efficient approximate mapping combined with Wave Front Alignment (WFA) to produce base-level alignments. This document focuses on the mapping component of WFMash, which enables the identification of homologous regions between genomes that can subsequently be aligned at the base level.
