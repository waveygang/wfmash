...
\title{MashMap: Fast Approximate Long-Read and Contig Mapper Using Minimizer Sketches}
...
\subsection{The Challenge of Sequence Mapping}

In the field of genomics, sequence mapping is a fundamental computational task that involves locating the position of a query sequence within a reference genome. This problem is of paramount importance in various applications, including genome assembly, variant calling, metagenomics, and comparative genomics. As sequencing technologies continue to evolve, producing longer reads with higher error rates, the challenge of efficient and accurate mapping becomes increasingly complex.

Traditional mapping algorithms often rely on exact matching approaches, which work well for short reads with low error rates. However, these methods face significant challenges when dealing with long reads or contigs, which may contain higher error rates, structural variations, or evolutionary divergence from the reference. Additionally, the exponential growth of genomic data requires scalable solutions that can handle large datasets without compromising accuracy.

\subsection{Motivation for Approximate Mapping}

Approximate mapping addresses these challenges by focusing on identifying regions of high similarity rather than exact matches. This approach offers several advantages:

\begin{itemize}
    \item \textbf{Efficiency}: By using dimensionality reduction techniques like minimizer sketching, approximate mapping algorithms can significantly reduce the computational overhead compared to exact alignment methods.
    
    \item \textbf{Robustness}: Approximate mapping is less sensitive to sequencing errors, structural variations, and evolutionary divergence, making it suitable for diverse applications.
    
    \item \textbf{Scalability}: The reduced computational requirements allow for processing of larger datasets, which is crucial for analyzing complex genomes or metagenomic samples.
\end{itemize}

\subsection{The MashMap Approach}

MashMap is an approximate mapping algorithm designed specifically for long reads and contigs. It combines efficient minimizer sketching with a sophisticated two-stage mapping process to achieve both speed and accuracy. The key innovations of MashMap include:

\begin{itemize}
    \item \textbf{Minimizer-based sketching}: Instead of comparing entire sequences, MashMap uses a k-mer minimizer approach to create compact "sketches" of sequences, dramatically reducing the computational burden.
    
    \item \textbf{Two-stage mapping}: The algorithm employs a two-stage approach, where the first stage (L1) identifies candidate regions using minimizer matching, and the second stage (L2) refines these candidates to obtain precise mapping coordinates.
    
    \item \textbf{Adaptive filtering}: MashMap implements various filtering strategies to retain only the most significant mappings, balancing sensitivity and specificity.
    
    \item \textbf{Chaining and merging}: For split-read mapping, MashMap employs chaining and merging techniques to combine fragmented mappings into coherent alignments.
\end{itemize}

This document provides a comprehensive description of the MashMap algorithm, detailing the mathematical foundations, implementation strategies, and parameter considerations for optimal performance across different applications.
...
\begin{algorithmic}[1]
\Procedure{SelectMinimizers}{$sequence, k, w$}
...
\For{each $reference$ in $references$}
    \State $minimizers \gets$ \Call{SelectMinimizers}{$reference, k, w$}
    \For{each $(minimizer, position)$ in $minimizers$}
