...
void WFlign::wflign_affine_wavefront(
    ...
    if (
        ...
        // Thread-reused aligner (penalties are run constants; align calls fully
        // reset per-alignment state). The previous record's patching may have
        // set a step limit on it, so restore the fresh default.
        thread_local std::unique_ptr<wfa::WFAlignerGapAffine2Pieces> standard_aligner;
        if (!standard_aligner) {
            standard_aligner = std::make_unique<wfa::WFAlignerGapAffine2Pieces>(
                        0,
        ...
        wfa::WFAlignerGapAffine2Pieces* wf_aligner = standard_aligner.get();
        ...
        thread_local std::unique_ptr<wfa::WFAlignerGapAffine> wflambda_aligner_tl;
        if (!wflambda_aligner_tl) {
            wflambda_aligner_tl = std::make_unique<wfa::WFAlignerGapAffine>(
        ...
        wfa::WFAlignerGapAffine* wflambda_aligner = wflambda_aligner_tl.get();
        ...
        thread_local std::unique_ptr<wfa::WFAlignerGapAffine> segment_aligner_tl;
        if (!segment_aligner_tl) {
            segment_aligner_tl = std::make_unique<wfa::WFAlignerGapAffine>(
        ...
        wfa::WFAlignerGapAffine* wf_aligner = segment_aligner_tl.get();
        wf_aligner->setHeuristicNone();
...
//        extend_data.wfa_affine_penalties = &wfa_affine_penalties;
        ...
        if (!trace.empty()) {
            ...
            if (merge_alignments) {
                ...
                thread_local std::unique_ptr<wfa::WFAlignerGapAffine2Pieces> patch_aligner_tl;
                if (!patch_aligner_tl) {
                    patch_aligner_tl = std::make_unique<wfa::WFAlignerGapAffine2Pieces>(
                                0,
                                wfa_convex_penalties.mismatch,
                                wfa_convex_penalties.gap_opening1,
                                wfa_convex_penalties.gap_extension1,
                ...
                wfa::WFAlignerGapAffine2Pieces* wf_aligner = patch_aligner_tl.get();
                wf_aligner->setHeuristicNone();
