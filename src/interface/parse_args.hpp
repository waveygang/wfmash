...
void parse_args(int argc,
	...
	args::ValueFlag<std::string> pairs_file(mapping_opts, "FILE", "file containing allowed query<TAB>target pairs (TSV, one pair per line)", {"pairs-file"});
	...
	if (pairs_file) {
		map_parameters.pairs_file = args::get(pairs_file);
	}
