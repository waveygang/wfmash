...
namespace skch
{
  // Fragment and output structures are now defined in fragmentManager.hpp
  ...
  class Map
  {
    ...
    private:
      ...
      void mapQuery() {
          ...
          // Store references to query outputs instead of copying mappings
          std::vector<std::shared_ptr<QueryMappingOutput>> allQueryOutputs;
          std::mutex allQueryOutputs_mutex;
          ...
          for (size_t subset_idx = 0; subset_idx < target_subsets.size(); ++subset_idx) {
              ...
              // No longer need subset mappings - we store QueryMappingOutput directly
              ...
              auto processQueries_task = subset_flow->emplace([this, progress, &allQueryOutputs, &allQueryOutputs_mutex,
                  ...
                  for (const auto& queryName : querySequenceNames) {
                      ...
                      free(seq_data); // Free the raw data after copying
                          ...
                          auto query_task = sf.emplace([&, queryName, sequence](tf::Subflow& query_sf) {
                              ...
                              if (param.filterMode == filter::ONETOONE) {
                                  // For ONETOONE mode, store the output object for later processing
              ...
              // No merge needed - QueryMappingOutputs are stored directly in allQueryOutputs
              auto merge_task = subset_flow->emplace([]() {
                  // No-op - outputs are already collected
              }).name("merge_results");
          ...
          if (param.filterMode == filter::ONETOONE && !exit_after_indices) {
              ...
              auto final_task = final_flow.emplace([&]() {
                  ...
                  size_t total_queries = allQueryOutputs.size();
                  
                  for (const auto& output : allQueryOutputs) {
                      total_mappings += output->results.getSize();
                  }
                  
                  std::cerr << "[wfmash::mashmap] Processing " << total_mappings 
                            << " mappings from " << total_queries 
                            << " queries for one-to-one filtering" << std::endl;
                  
                  // NEW: Process compressed mappings directly without expanding all at once
                  // Structure: ref_seqId -> vector of {queryOutput, mapping_index}
                  struct CompressedMappingRef {
                      std::shared_ptr<QueryMappingOutput> output;
                      size_t index;
                  };
                  std::unordered_map<seqno_t, std::vector<CompressedMappingRef>> targetMappingRefs;
                  
                  // Collect references to compressed mappings by target
                  for (const auto& output : allQueryOutputs) {
                      auto compressedMappings = output->results.getCompressedMappings();
                      for (size_t i = 0; i < compressedMappings.size(); ++i) {
                          targetMappingRefs[compressedMappings[i].ref_seqId].push_back({output, i});
                      }
                  }
                  
                  // Use a progress meter for the filtering step
                  auto filterProgress = std::make_shared<progress_meter::ProgressMeter>(
                      targetMappingRefs.size(), 
                      "[wfmash::mashmap] One-to-one reference filtering",
                      param.use_progress_bar);
                  
                  // Write output directly without storing all filtered mappings
                  std::ofstream outstrm(param.outFileName);
                  size_t final_mapping_count = 0;
                  
                  // Process each reference's mappings
                  for (auto& [targetSeqId, mappingRefs] : targetMappingRefs) {
                      // Only expand mappings for this reference temporarily
                      MappingResultsVector_t refMappings;
                      refMappings.reserve(mappingRefs.size());
                      
                      for (const auto& ref : mappingRefs) {
                          auto compressedMappings = ref.output->results.getCompressedMappings();
                          seqno_t qSeqId = idManager->getSequenceId(ref.output->queryName);
                          offset_t qLen = idManager->getSequenceLength(qSeqId);
                          
                          MappingResult expanded = expandMinimalMapping(compressedMappings[ref.index], qSeqId, qLen);
                          refMappings.push_back(expanded);
                      }
                      
                      // Apply filtering for this reference
                      MappingResultsVector_t filteredMappings;
                      filterByGroup(refMappings, filteredMappings, param.numMappingsForSegment - 1, 
                                   true, *idManager, *filterProgress);
                      
                      // Write filtered mappings immediately grouped by query
                      std::unordered_map<seqno_t, MappingResultsVector_t> queryGrouped;
                      for (auto& mapping : filteredMappings) {
                          queryGrouped[mapping.querySeqId].push_back(mapping);
                      }
                      
                      for (auto& [querySeqId, mappings] : queryGrouped) {
                      ...
                      // Memory freed automatically when refMappings and filteredMappings go out of scope
                  }
...
VecIn mergeMappingsInRange(VecIn &readMappings,
    ...
    return maximallyMergedMappings;
      ...
      std::pair<MappingResultsVector_t, MappingResultsVector_t> filterSubsetMappings(MappingResultsVector_t& mappings, progress_meter::ProgressMeter& progress) {
          ...
          // CRITICAL: DO NOT COPY! Work with the original mappings
          ...
          if (param.mergeMappings && param.split) {
              ...
              if (param.scaffold_gap > 0 || param.scaffold_min_length > 0 || param.scaffold_max_deviation > 0) {
                  filterByScaffolds(maximallyMergedMappings, maximallyMergedMappings, param, progress);
              }
          } else {
     ...
     /**
       ...
       */
      // Direct output of compressed mappings without creating full vectors
      void reportCompressedMappings(const std::vector<MinimalMapping>& compressedMappings,
                                   ...
                                   std::ostream& outstrm)
      {
        ...
        // For chain assignment, we need to expand mappings temporarily
        // but we can process them in smaller batches
        const size_t BATCH_SIZE = 1000;
        
        for (size_t start = 0; start < compressedMappings.size(); start += BATCH_SIZE) {
            size_t end = std::min(start + BATCH_SIZE, compressedMappings.size());
            
            // Output batch directly without storing
            for (size_t i = start; i < end; ++i) {
                const auto& compressed = compressedMappings[i];
                
                // Expand only for output
                float identity = compressed.identity / 100.0f;
                float fakeMapQ = identity == 1 ? 255 : std::round(-10.0 * std::log10(1-identity));
                std::string sep = param.legacy_output ? " " : "\t";
                
                outstrm << queryName
                        << sep << queryLen
                        << sep << compressed.query_pos
                        << sep << (compressed.query_pos + compressed.length) - (param.legacy_output ? 1 : 0)
                        << sep << (compressed.getStrand() == strnd::FWD ? "+" : "-")
                        << sep << idManager->getSequenceName(compressed.ref_seqId)
                        << sep << idManager->getSequenceLength(compressed.ref_seqId)
                        << sep << compressed.ref_pos
                        << sep << (compressed.ref_pos + compressed.length) - (param.legacy_output ? 1 : 0);
                
                if (!param.legacy_output) {
                    // For compressed mappings, we don't store all metadata
                    // Use reasonable defaults
                    outstrm << sep << 0 // conservedSketches - would need to store
                            << sep << compressed.length // blockLength
                            << sep << fakeMapQ
                            << sep << "id:f:" << identity
                            << sep << "kc:f:" << 0; // kmerComplexity - would need to store
                    
                    if (!param.mergeMappings) {
                        outstrm << sep << "jc:f:" << 0; // jaccard - would need to store
                    } else {
                        // Chain info would need separate tracking
                        outstrm << sep << "ch:Z:" << i << ".1.1"; // Simple chain per mapping for now
                    }
                } else {
                    outstrm << sep << identity * 100.0;
                }
